...
namespace ImGui {
    ...
    void GetNeoSequencerZoomedRange(uint32_t* zoomedStartFrame, uint32_t* zoomedEndFrame) {
        ImGuiWindow *window = GetCurrentWindow();
        const auto id = window->IDStack[window->IDStack.size() - 1];
        const auto &context = sequencerData[id];
        const uint32_t totalFrames = context.EndFrame - context.StartFrame;
        const auto viewWidth = (uint32_t)((float)totalFrames / context.Zoom);
        *zoomedStartFrame = context.StartFrame + context.OffsetFrame;
        *zoomedEndFrame = context.StartFrame + context.OffsetFrame + viewWidth;
    }

	bool
		BeginNeoSequencer(const char* idin, uint32_t* frame, uint32_t* startFrame, uint32_t* endFrame, const float zoom, const bool externalFrameUpdate, const ImVec2& size,
        ...
        if (!context.HoldingZoomSlider && externalFrameUpdate) {
            uint32_t zoomedStartFrame = 0, zoomedEndFrame = 0;
            GetNeoSequencerZoomedRange(&zoomedStartFrame, &zoomedEndFrame);
            uint32_t zoomedRangeSize = zoomedEndFrame - zoomedStartFrame;
            if (*frame < zoomedStartFrame) {
                context.OffsetFrame =std:: min(*frame, *endFrame - zoomedRangeSize);
            } else if (*frame > zoomedEndFrame) {
                context.OffsetFrame = *frame - zoomedRangeSize;
            }
        }
